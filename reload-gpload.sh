...
##need change
GP_DBHOST=`grep GP_DBHOST tpcds.config|awk -F '=' '{print $2}'`

#type 'p' table ORIENTATION is parquet, 'o' table format is orc ,'m' table format is magmaap
DDLTYPE=`grep DDLTYPE tpcds.config|awk -F '=' '{print $2}'`
DATASIZE=`grep DATASIZE tpcds.config|awk -F '=' '{print $2}'`
T_DATA_DIR=`grep DIRS tpcds.config|awk -F '=' '{print $2}'`
INSERT_STREAMS=`grep INSERT_STREAMS tpcds.config|awk -F '=' '{print $2}'`
ISGENDATA=`grep ISGENDATA tpcds.config|awk -F '=' '{print $2}'`
BUCKETNUM=`grep BUCKETNUM tpcds.config|awk -F '=' '{print $2}'`

time=""
CURDIR=`pwd`
rm -rf ${CURDIR}/logs/insert*

echo > logs/queryresult4tpcdsform.out
...
do
	...
	d_gpfdist_ips="${d_gpfdist_ips},'gpfdist://${i}:${T_PORT}/tpcds/${DATASIZE}/${TBNAME}/${TBNAME}*dat'"
...
if [ ${INSERT_STREAMS} -gt 1 ];then
	...
	for table in call_center catalog_page catalog_returns catalog_sales customer customer_address customer_demographics date_dim household_demographics income_band inventory item promotion reason ship_mode store store_returns store_sales time_dim warehouse web_page web_returns web_sales web_site
	do
	...
	for table in call_center catalog_page catalog_returns catalog_sales customer customer_address customer_demographics date_dim household_demographics income_band inventory item promotion reason ship_mode store store_returns store_sales time_dim warehouse web_page web_returns web_sales web_site
	do
		cat logs/insert$table.out >> logs/queryresult4tpcdsform.out
	done

	echo "total insert time : ${time} ms" >> logs/queryresult4tpcdsform.out
	...
	do
		printf 'Loading and analyzing table: %s ...' $table
		runGpload $table >> logs/queryresult4tpcdsform.out
	done
...
psql -c "Select relname,pg_size_pretty(pg_total_relation_size(oid)) from pg_class where relname in ('call_center','catalog_page','catalog_returns','catalog_sales','customer','customer_address','customer_demographics','date_dim','household_demographics','income_band','inventory','item','promotion','reason','ship_mode','store','store_returns','store_sales','time_dim','warehouse','web_page','web_returns','web_sales','web_site') order by pg_total_relation_size(oid) desc"
